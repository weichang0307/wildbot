...
class ControlNode(Node):
    ...
    _ADJUST_HDG_TOL    = 0.05   # rad — heading error considered "aligned"
    ...
    _PASS_HC_GAIN      = 0.05   # rad/s per unit of hc1-hc2 difference
    _PASS_HC_MAX_ANG   = 0.4    # rad/s maximum correction angular velocity
    _PASS_COAST_NS     = 1_000_000_000  # 0.2 s straight coast after hc lost

    def __init__(self):
        super().__init__('bridge_control_node')
        ...
        self.path_long_distance_threshold = 0.5
        ...
        self._exit_pose:     PoseStamped | None = None
        ...
        self.create_subscription(PoseStamped, '/bridge/exit',     self._on_exit,     10)
        self.create_subscription(Int32,       '/sensor/hc1',      self._on_hc1,      10)
        self.create_subscription(Int32,       '/sensor/hc2',      self._on_hc2,      10)
        ...
        self.get_logger().info("bridge_control_node — MANUAL. 'q' toggles AUTO, WASD to drive.")
    ...
    def _on_hc1(self, msg):
        if msg.data == 0:
            return
        if self._hc1 is None:
            self._hc1 = msg.data
            return
        self._hc1 = msg.data * self.hc_filter_alpha + (1 - self.hc_filter_alpha) * (self._hc1 or 0)

    def _on_hc2(self, msg):
        if msg.data == 0:
            return
        if self._hc2 is None:
            self._hc2 = msg.data
            return
        self._hc2 = msg.data * self.hc_filter_alpha + (1 - self.hc_filter_alpha) * (self._hc2 or 0)
    ...
    def _get_entry_tick(self):
        # Check if we've arrived at the entrance
    ...
    def _adjust_tick(self):
        ...
        if abs(heading_err) < self._ADJUST_HDG_TOL:
            if (now - self._steady_time).nanoseconds > self._ADJUST_STEADY_NS:
                ...
                self.get_logger().info('Aligned — PASS')
                return
        else:
    ...
    def _pass_tick(self):
        self.get_logger().info(f'PASS  hc1={self._hc1}  hc2={self._hc2}')
        ...
        # if math.hypot(rx - sx, ry - sy) >= self._PASS_DISTANCE:
        #     self._publish_wheel(0.0, 0.0)
        #     self.state = State.DONE
        #     self.get_logger().info('Bridge passed — DONE')
        #     return

        

        ang_vel = 0.0
        if self._hc1 is not None and self._hc2 is not None and self._hc1 < 200 and self._hc2 < 200:
            self._on_bridge = True
            diff = self._hc1 - self._hc2
            if abs(diff) > 5:
                ang_vel = max(-self._PASS_HC_MAX_ANG,
                          min(self._PASS_HC_MAX_ANG, -diff * self._PASS_HC_GAIN))
        elif self._on_bridge and self._hc1 > 200 and self._hc2 > 200:
            # Lost hc readings — coast straight for 1 s then switch to RETURN
            if self._coast_start is None:
                self._coast_start = self.get_clock().now()
                self.reinit_pub.publish(Empty())
            if (self.get_clock().now() - self._coast_start).nanoseconds < self._PASS_COAST_NS:
                
                self._publish_wheel(self._PASS_SPEED * 0.1, 0.0)
            else:
                self._publish_wheel(0.0, 0.0)
                self._enter_return()
            return
